	...
	api.ajax({
    	...
    	if(ret){
    		if(ret.status==1){
    			...
    			$('#party_name').html(ret.data.party_name);//党组织
    			...
    			if(ret.data&&ret.data.length>0)
	    			for(var i=0;i<ret.data.length;i++){
	    				$('#communist_list').append(
							'<li class="m-a pull-left mb-15em w-20b text-align" communistNo="'+ret.data[i].communist_no+'" onclick="openInfo(\'com_phonebook_info_header\','+ret.data[i].communist_no+','+nochat+')">'+
								'<img class="bor-ra-50b w-50em h-50em m-a" src="'+((ret.data[i].communist_avatar)?(localStorage.url+ret.data[i].communist_avatar):("../../../statics/images/images_oa/oa_img_head.png"))+'" alt="">'+
								'<div class="f-12em color-47 mt-5em">'+ret.data[i].communist_name+'</div>'+
							'</li>'
	    				)
	    			}
			}
