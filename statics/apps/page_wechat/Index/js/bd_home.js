					...
					if(ret.data&&ret.data.length>0){//图片列表存在并且有图片时					
						for(var i=0;i<ret.data.length;i++){												
							$('.swiper-wrapper').append(							
								'<div class="bor-ra-5em swiper-slide blue-slide pulse swiper-slide-active">'+'<img class="bor-ra-5em w-100b h-200em" src="'+localStorage.url+ret.data[i]+'" />'+
								'</div>'
    		...
    		if(ret.status==1){
    			if(ret.data&&ret.data.length>0){
	    			for(var i=0;i<((ret.data.length<=10)?(ret.data.length):(10));i++){
		    			$('#list').append(
		    				'<li class="va-jz over-h bb-1-f0 pb-12em pt-12em" onclick="openPartyWorkInfo(\'cms_partyWork_info_header\','+ret.data[i].article_id+')">'+
