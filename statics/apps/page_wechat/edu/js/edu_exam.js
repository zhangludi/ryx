	...
	api.ajax({
		...
		data: {//传输参数
			values: {
				...
				"is_exam": is_exam,// 1 正式考试未考  2  所有已经考过的模拟和正式考试 3 未考的模拟考试
				"page": page,
				"pagesize": 10,
			}
		}
			...
			if (ret.status == 1) {
				...
				if (is_exam == 3) {
					...
					for (var i = 0; i < data.length; i++) {
						...
						<li onclick="openExamList('edu_exam_info',${data[i].exam_id})">
							...
							<div class="model_tests_list_bot">
								...
								<span class="model_tests_list_status not_red">${data[i].join_val}</span>
							</div>
						</li>
					</ul>`
						)
					...
					for (var i = 0; i < data.length; i++) {
						$('.model_join').append(
							`<ul class="model_join_list">
								...
								<a href="javascript:;" class="model_join_list_status not_red"><span>${data[i].score}分</span></a>
								</div>
								</li>
							</ul>`
						)
