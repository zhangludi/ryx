	...
	api.ajax({
    	...
    	if(ret){
    		if(ret.status==1){
    			...
    			$('#add_type').html(ret.data.material_topic.substring(0,10));//时间
    			...
    			if(ret.data.notes_list){
    				for(var i=0;i<ret.data.notes_list.length;i++){
    					 $('#list1').append(
    					 	'<li class="border-3em-de pt-12em pb-12em" onclick="openScene_infowin(\'edu_special_twostudies_scene_info_header\','+ret.data.notes_list[i].notes_id+')">'+
    					 		'<div class="f-12em color-21">'+ret.data.notes_list[i].add_time+'</div>'+
    					 		'<div class="f-12em color-66 pt-15em">'+ret.data.notes_list[i].notes_content+"……"+
    					 		'<span class="f-12em color-d4a500">'+"[更多]"+'</span>'+
    					 		'</div>'+    					 	
    					 	'</li>'
    					 )
    				}
