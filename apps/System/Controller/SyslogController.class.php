...
class SyslogController extends BaseController
{
	...
	/**
	 * @name  sys_log_index_data()
	 *  @desc  意见反馈列表数据
	 * @author 杨凯
	 * @version 版本 V1.0.0
	 * @addtime   2018-6-26
	 */
	public function sys_log_index_data(){
	    $opinion_type = I('get.opinion_type');
	    ...
	    foreach($log_list as &$log){
	        $log['add_communist_name'] = getStaffInfo($log['add_staff']);
	    }
	    $data['data'] = $log_list;
	    $data['count'] = $log_num;
		$data = array(
	...
	public function sys_log_del(){
	    ...
	    if($log_del){
	        showMsg('success', '操作成功！',U('sys_log_index'));
	    }else{
	        showMsg('error', '操作失败！');
	    }
	}
