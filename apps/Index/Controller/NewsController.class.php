...
class NewsController extends Controller
{
    ...
    {
        //轮播
        $where['article_point'] = 2;
        $article = M('cms_article')->where($where)->field('article_id,article_title,article_thumb')->limit(3)->order('add_time desc')->select();
        ...
        //新闻排行
        $article_ranking  = M('cms_article')->field('article_id,article_title')->limit(6)->order('article_view desc')->select();
        $this->assign('article_ranking',$article_ranking);
    	// 获取新闻动态列表
  ...
  //       // dump($meetting_list);die;
    ...
    /**
     *  ipam_meeting_index_ajax_count
     * @desc 新闻列表ajax
     * @user 刘长军  
     * @date 2019/8/28
     * @version 1.0.0
     */
    public function ipam_meeting_index_ajax_count(){
        $cat_id = I('post.cat_id');
        $where['article_cat'] = $cat_id;
        $article_count = M('cms_article')->where($where)->count('article_id');
        ob_clean();$this->ajaxReturn($article_count);
    }
    /**
     ...
     * @version 1.0.0
     */
    public function ipam_news_index_ajax(){
        $pagesize = I('post.pagesize');
        $where['article_cat'] = I('post.cat_id');
        $article_list = M('cms_article as a')->field("a.article_id,a.article_title,a.article_source,a.article_description,a.article_thumb,a.add_time")->limit($pagesize)->where($where)->select();
        $db_give = M('cms_article_give');
        $db_comment= M('cms_article_comment');
        foreach ($article_list as &$value) {
            $value['article_title_t']  = mb_substr($value['article_title'],0,35);
            $value['give_count'] = $db_give->where("article_id=".$value['article_id']." and give_type=1")->count("give_id");
            $value['fav_count'] = $db_comment->where("article_id=".$value['article_id']."")->count("comment_id");
            $value['article_thumb'] = getUploadInfo($value['article_thumb']);
        }
        $data = '';
        foreach ($article_list as $list) {
            $data.='<li onclick="article_info('.$list['article_id'].')">
                    <div class="tit" title='.$list['article_title'].'>'.$list['article_title'].'</div>
                    <div class="clearfix mt-20">
                        <div class="pull-left po-re">
                            <img src="'.$list['article_thumb'].'" style="width:183px;height:102px">
                        </div>
                        <div class="pull-right w-72">
                            <div class="tit2">'.$list['article_description'].'</div>
                            <div class="clearfix">
                                <div class="pull-left address color-9e f-16em mr-10">来源：'.$list['article_source'].'</div>
                                <div class="pull-left time color-9e f-16em">'.$list['add_time'].'</div>
                                <div class="pull-right va-jz color-9e f-16em">
                                    <div class="mr-15 di-i">
                                        <img class="di-ib mr-5 mb-3" src="../../../statics/apps/page_index/images/icon_zf.png"/>209
                                    </div>
                                    <div class="mr-15 di-i">
                                        <img class="di-ib mr-5 mb-3" src="../../../statics/apps/page_index/images/icon_pl.png"/>'.$list['fav_count'].'
                                    </div>
                                    <div class="di-i">
                                        <img class="di-ib mr-5 mb-3" src="../../../statics/apps/page_index/images/info_dz2.png"/>'.$list['give_count'].'次
                                    </div>
                                </div>
                            </div>
    ...
    public function ipam_news_detail() {
        ...
        foreach ($comment_data as &$value) {
			$value['communist_avatar'] = getCommunistInfo($value['communist_no'],'communist_avatar');
			if(!$value['communist_avatar'] || !file_exists(SITE_PATH.$value['communist_avatar'])){
    	...
    	if(!empty($article_id)){
            ...
            $this->assign('article_id',$notice_id);
    	}
